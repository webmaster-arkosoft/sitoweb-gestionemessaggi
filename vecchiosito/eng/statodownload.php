<?php
	if(isset($_GET['idutente']) and strlen($_GET['idutente'])>0 and isset($_GET['codice']) and strlen($_GET['codice'])>0){
		...
		$query=mysql_query("select * from `statodownload-eng` where idutente='$idutente' and codice='$codice'") or die ("Query controllo non eseguita!");
		//conto i prodotti inseriti
		$righe=mysql_num_rows($query);
		if($righe==1){
			mysql_query("UPDATE `statodownload-eng` SET statodownload='1' where idutente='".$idutente."'");
			//ricavo dati
			$query1=mysql_query("select * from `download-eng` where id='$idutente'") or die ("Query controllo non eseguita!");
